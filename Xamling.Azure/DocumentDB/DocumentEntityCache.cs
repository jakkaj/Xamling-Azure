...
namespace Xamling.Azure.DocumentDB
{
...

    {
        ...
        public async Task<List<T>> QueryEntity<T>(string query)
            where T : class, new()
        {
            var repo = _getRepo<T>();
        ...
        public async Task<List<T>> Query<T>(IQueryable<XDocumentCacheItem<T>> query) where T : class, new()
        {
            var repo = _getRepo<T>();

            var result = await repo.Query(query);
